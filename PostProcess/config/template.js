...
vec3 rectMask(vec2 texCoord, vec2 p1, vec2 p2, vec3 color1, vec3 color2) {
    vec2 minP = min(p1, p2);
    vec2 maxP = max(p1, p2);
...
vec3 ellipseMask(vec2 texCoord, vec2 p1, vec2 p2, vec3 color1, vec3 color2) {
    vec2 minP = min(p1, p2);
    vec2 maxP = max(p1, p2);
...
{
  "pack": {
    "description": "Generated By BlueMango Shader Generator. (https://mangojun.github.io/bluemango/PostProcess)",
    "min_format": 75,
    "max_format": 75
  }
}
